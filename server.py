...
async def kasra_get_cartable_items(
    ...
    """
    مشاهده اسناد و درخواست‌های کارتابل گردش کار (اسناد دریافتی برای بررسی، تایید یا ارجاع).
...
async def kasra_get_credit_types(reason: Optional[str] = None) -> Dict[str, Any]:
    """
    دریافت لیست کامل انواع مجوز‌ها، مرخصی‌ها، مأموریت‌ها و اضافه‌کارها با کدهای سیستمی آنها.
    دستهها شامل:
...
async def kasra_get_submitted_requests(
    ...
    """
    دریافت لیست درخواست‌های ثبت‌شده (مجوز‌ها، مرخصی‌ها، مأموریت‌ها) و وضعیت تایید/رد آنها در بازه زمانی.

    Args:
        start_date: تاریخ شروع شمسی (پیشفرض: 1404/01/01)
        end_date: تاریخ پایان شمسی (پیشفرض: 1405/12/29)
        reason: دلیل اختیاری
    """
    client = _get_client()
...
async def kasra_submit_credit_request(
    ...
    Args:
        ...
        is_daily: True اگر روزانه باشد، False اگر ساعتی باشد. در صورت تعیین‌نشدن، بر اساس پر بودن ساعتها تشخیص داده می‌شود.
        reason: دلیل اختیاری
    """
    client = _get_client()
...
async def kasra_delete_credit_request(
    ...
    Args:
        ...
        doc_type_id: شناسه نوع سند (پیشفرض 1 برای ساعتی، 2 برای روزانه)
...
async def kasra_get_desktop_shortcuts(reason: Optional[str] = None) -> Dict[str, Any]:
    """
    دریافت میانبرهای میز کار و صفحات برگزیده کاربر.
